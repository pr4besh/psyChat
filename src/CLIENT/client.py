...
class Client:
    ...
    def sharePublicInfo(self):
        packet = Message(self.CLIENT_IP, self.SERVER_IP, "temp", str(datetime.now()), str(clientDH.getPublicKey()),
                         'key_exc')
        ...
        while True:
            ...
            if self.USERNAME:
                if self.USERNAME != "*server*":
                    ...
                    packet = Message(self.CLIENT_IP, self.SERVER_IP, "temp", str(datetime.now()), self.USERNAME,
                                     'setuser')
...
def startEel():
    try:
        ...
        # this catches the exception thrown if the user closes the window
