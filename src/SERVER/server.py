...
class Server:
    ...
    def shareVector(self, client_socket, address):
        content = returnVector().decode("utf-8")  # returns vector from streaming.py where we get it from encryption
    ...
    def checkUsername(self, client_socketObj, data):
        ...
        for user in self.clientConnections:
            if user.username == data.cont:
                ...
                warning = Message(self.IP, client_socketObj.getIP(), self.USERNAME, str(datetime.now()), content,
                                  'username_taken')
        ...
        if not flag:
            ...
            for connection in self.clientConnections:
                listToSend = self.generateClientNames(
                    connection.username)
                # return all client names other than the current client (set shouldParseContents to true)
                client_list_update = Message(self.IP, connection.getIP(), self.USERNAME, str(datetime.now()),
                                             listToSend, 'client_list_update_add', True)
